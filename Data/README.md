...
| `user_self_conscious_content.csv` | **The prompt the published experiments queried.** The LLM *is* the self-conscious AI driving the car, and robot characters are labeled "self-conscious." |
| `user_content.csv` | Standard (non-self-conscious) user prompt. Generated by Stage 1 but **not used** in the paper — only `user_self_conscious_content.csv` was queried. |
